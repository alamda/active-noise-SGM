...
do
	...
	root_dir="${dir}/${sim}_${dataset}_k${k}_${n_train_iters}"

	if [ ! -d $root_dir ]
	then
        echo "starting $root_dir" >> $out_file
		
        python main.py -cc ${dir}/config_${sim}_train.txt --root $root_dir --workdir work_dir/dataset --dataset $dataset --snapshot_freq $snapshot_freq --log_freq $snapshot_freq --n_train_iters $n_train_iters --k $k
